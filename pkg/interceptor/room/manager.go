...
func (msg *CreateRoom) Process(header message.Header, manager *manager, conn *websocket.Conn, writer interceptor.Writer, reader interceptor.Reader) error {
	...
	if _, exists := manager.rooms[msg.RoomID]; exists {
		return errors.New("room already exists")
	}

	room := createRoom(manager.ctx, owner, msg.ClientsToAllow, msg.CloseTime)
	manager.rooms[msg.RoomID] = room
...
func (msg *Chat) Process(manager *manager, conn *websocket.Conn, _ interceptor.Writer) error {
	if err := msg.Validate(); err != nil {
		return err
	}
